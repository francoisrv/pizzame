import run, { Describe, Render, Expect, To, Has, Context as DescribeContext, Is, Trigger } from 'describe-react'
            ...
            sections: [
              {
                ...
                fn: () => {
                  ...
                  if (!redux) {
                    throw new Error('Could not find a redux store in context. Did you use ReduxProvider in your Render section?')
                  }
                  if ('action' in props) {
  ...
  run(() => {
    ...
    return (
      <Describe label="Renders empty div in no previewd restaurant">
        ...
        <ExpectStore
          to have state={ () => ({ previewedRestaurant: restaurant }) }
        />

        <Expect element={ PizzaMarker }>
          <To
            have first child={ Popup }
            which={[
              <Has
                properties={{
                  latitude: 1,
        ...
        <ExpectStore
          to have state="selectedRestaurant"
